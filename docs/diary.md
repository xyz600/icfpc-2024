...
efficiency の problem 2 は mul 0 があったので、そのための対策を入れてしまった。
...
になっていて、不動点検出ができるらしい。
不動点が簡約できるとめちゃ嬉しいことになるので、頑張る。

Add(1, Add(1, Add(1, ....)))

が大量にある。とりあえず木が無限に深くならないような対応を入れた Add(1, Add(1, _)) => Add(_, Add(1, 1)) みたいなやつ。
けど、ラムダ式の apply をどうにも固定サイズのメモリでうまくやる方法がわからなくて、結局終わらん。

## 16:47

いやーーーーーーーーーーーどうしたらいいか何もわからん。
Yコンビネータを決め打ってパーサーに加えたらいいとかあるのかな。

## 17:56

ほえー難しいなぁ。
efficiency どうしてもやりたかったけど難しそうだから、spaceship やるか…
結局、メモ化をどうしたらいいのか何もわからなかったという話になるけど、まとまった資料や実装もあまり見つからなかったから、ちゃんと勉強したい…

## 18:35

lambdaman で何でこんだけスコア低いのかなーということを少し考えてた。
多分、無限再帰(に近い動き)でひたすら歩き続けるパターンを生成する方法があるんだろうな。
少し考えてみる。

## 18:53

何もわからんので、spaceship ソルバ書くか。
